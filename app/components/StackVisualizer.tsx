                      ...
                      <div className="flex flex-col gap-2 p-3 bg-black/40 rounded-xl border border-white/5 group-hover:border-blue-500/30 transition-all">
                        ...
                        <div className="flex flex-col gap-1.5">
                          ...
                          <div className="flex items-center justify-between px-2 py-1 bg-black/40 rounded border border-white/5" id={v.type === 'pointer' ? `ptr-src-${v.id}` : undefined}>
                            ...
                            <div className={`text-[10px] font-mono font-bold flex-1 text-right truncate pl-4 ${v.type === 'pointer' ? 'text-emerald-400 underline decoration-emerald-500/30 underline-offset-4' : 'text-slate-200'}`}>
                              {v.value.startsWith('{') ? (
                                <div className="text-left bg-black/20 p-2 rounded border border-white/5 space-y-1 mt-1 max-h-32 overflow-y-auto custom-scrollbar">
                                  {v.value.replace(/[{}]/g, '').split(',').map((part, idx) => (
                                    <div key={idx} className="flex justify-between border-b border-white/5 last:border-0 pb-1">
                                      <span className="text-[8px] text-blue-400/80">{part.split(':')[0].trim()}</span>
                                      <span className="text-emerald-400">{part.split(':')[1]?.trim() || '0'}</span>
                                    </div>
                                  ))}
                                </div>
                              ) : (
